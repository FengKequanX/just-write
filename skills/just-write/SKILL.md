...
```
【截图来源清单】
| # | 保存文件名 | 来源 URL / 获取方法 | 截图要点提示 |
|---|-----------|-------------------|-------------|
| 1 | imgs/xxx.png | https://xxx | 截取包含XX数据的部分 |
```

要求：
- 每个 `![描述](路径)` 占位符都必须有对应的来源，且文件名一致
- URL 必须是在前置调研阶段已验证过的真实链接
- 截图要点提示要具体（如"截取表格第3行数据"而非"截取页面"）
- 如需登录或特殊操作才能获取，说明具体步骤
- 用户按清单中的文件名保存截图即可，无需额外沟通文件名
...
截图来源清单已在内容确认后提供，用户按清单文件名保存截图。检查 `imgs/` 目录，确认所有截图文件已到齐即可。
