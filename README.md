...
- **封面页** — 使用文章 frontmatter 中的 `coverImage`（兜底读取 `cover.png` 或 `imgs/cover.png`）+ 文章标题 + 作者。封面图按真实比例完整显示，不裁切；**4:3** 封面图在小红书 3:4 轮播中首屏占比更佳。
- **内容页** — 按文章顺序连续排版，H2/H3 标题作为页内标题显示。分页按视觉高度自动拆分，保持每页内容饱满且不溢出。
