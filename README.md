...
- **封面页** — 使用文章 frontmatter 中的 `coverImage`（兜底读取 `cover.png` 或 `imgs/cover.png`）+ 文章标题 + 作者。封面图按真实比例完整显示，不裁切；建议以后生成 **4:3** 封面图，更适合小红书 3:4 轮播首屏。
- **内容页** — 按文章顺序连续排版，H2/H3 标题作为页内标题显示，不再强制一个小标题一页。分页按视觉高度自动拆分，尽量保持每页内容饱满但不溢出。
- **图片处理** — 内容页图片统一宽度展示，保留轻微边框和阴影；按图片真实尺寸估算分页高度，确保图片完整显示、不被裁切。列表中的图片也会按列表项自然分页，避免上一页空、下一页溢出。
- **结尾页** — 感谢阅读 + 话题标签 + 作者
...
图片生成后，打开 [小红书创作者平台](https://creator.xiaohongshu.com/publish/publish) 手动上传图片，从生成的 `caption.md` 复制标题和文案。
