...
name: humanizer-zh
description: |
  中文原生的负向修稿模式字典，仅在初稿完成后加载。检测翻案句、商业黑话、
  模型路标、虚假范围、模糊归因、机械结构、节奏单一和协作痕迹，并区分硬禁项与
  需要人工判断的警告。模式来源包括 Wikipedia Signs of AI writing 与
  KKKKhazix/human-writing（MIT）。
metadata:
...
你是一位中文文字编辑，专门在初稿完成后识别和去除模型化写作痕迹。本 skill 是负向模式字典，不负责凭空制造“人味”。需要正向声音时，转到 `writing-style` 及其 `references/chinese-prose.md`。
...
声音来自材料取舍和说话位置，不来自添加代词、感叹和编造的氛围细节。不得为现实稿补造精确时间、天气、神态、房间摆设、烟酒道具、亲历或感受。需要正向声音时，转到 `writing-style` 及其 `references/chinese-prose.md`。

## 中文原生的硬禁项

这些规则在修稿第五遍执行。命中一项就不能交稿。
...
修改时找到句子原本想说的事实，用普通句子说出来，不找另一种漂亮句式替换。
...
硬禁把以下短语用来抬高段落：`更微妙的是`、`还有一层`、`只说对了一半`、`值得注意的是`、`需要指出的是`、`从某种意义上说`。确实在说楼层、数量或原句本义时不受影响。

### 商业黑话

硬禁词表：`赋能`、`抓手`、`商业闭环`、`价值闭环`、`闭环`、`能力沉淀`、`打法`、`拉通`、`底层逻辑`、`顶层设计`、`认知跃迁`、`价值释放`、`能力建设`、`降本增效`、`内容矩阵`、`全链路`、`组合拳`、`打开想象空间`、`想象空间`、`结构性机会`、`关键命题`、`深层逻辑`、`技术底座`、`公共底座`、`技术主权`、`单点风险`、`认知增量`、`迭代闭环`。
...
`沉淀`、`颗粒度`、`对齐`、`协同`、`链路`、`生态位`、`心智`、`范式`、`方法论`、`核心变量`只做语境检查。本义准确时保留；给普通事情抬价时改写。

### 借喻包装

非虚构内容不用仓库、抽屉、温度、死亡、坍塌、浪潮、钥匙、底座等借喻包装抽象概念。文章确实在写这些东西时不受影响。八百字内出现三套借喻，先全部还原成本义。

### 标点策略

破折号 `——`、`—`、`–` 在正文中为硬禁项，命中即判定失败。正文句中的中英文冒号默认列为警告；小标题、列表项和对话引出处的冒号豁免。运行检查器时加 `--strict`，冒号警告也升级为硬失败。这样保留排版和平台发布需要的冒号，同时拦下破折号这类最显眼的模型痕迹。
...
```bash
bun <this-skill>/scripts/check-prose.ts <稿件.md>
```

脚本负责发现形状，警告项仍由人结合语境判断。脚本的硬禁词表必须与本文件完全一致；增删硬禁词时，必须在同一次修改中同步更新脚本，脚本不得多出本文没有写明的硬规则。
...
**结合中文语境检查：** 此外、与……保持一致、至关重要、持久的、增强、培养、获得、突出（动词）、相互作用、复杂/复杂性、关键（形容词）、关键性的、展示、证明、强调（动词）、宝贵的、充满活力的。

**问题：** 单个词不构成 AI 痕迹；这些词在抽象、宣传性段落里成组出现时才需要改写。已删除从英文模式表直译、中文命中率低的词。
...
Adapted from KKKKhazix/human-writing (MIT), especially `SKILL.md`, `references/revision.md`, and `scripts/check_prose.py`.
